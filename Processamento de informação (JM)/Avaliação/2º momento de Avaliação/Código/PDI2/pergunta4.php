<?php
    ...
    ROUND((COUNT(DISTINCT h.id_docente) / (SELECT COUNT(DISTINCT id_docente) FROM habilitacoes) * 100), 2) AS Percentagem_especialidade
FROM
    habilitacoes h
LEFT JOIN
    especialidade e ON h.id_especialidade = e.id
GROUP BY
    e.especialidade;";
    ...
    foreach ($arr as $item) {
        $labels[] = $item['especialidade'];
        $data[] = $item['Percentagem_especialidade'];
